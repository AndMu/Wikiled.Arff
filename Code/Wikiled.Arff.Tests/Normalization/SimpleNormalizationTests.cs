...
namespace Wikiled.Arff.Tests.Normalization
{
    [TestFixture]
    public class SimpleNormalizationTests
    {
        ...
        public void Windowed()
        {
            ...
            var result = data.Windowed(2).ToArray();
        ...
        {
            double[] data = { 1, 2, 2, 3, 4, 10 };
            var result = data.WindowedEx(2, null).ToArray();
            Assert.AreEqual(5, result.Length);
            Assert.AreEqual(1, result[0][0]);
            Assert.AreEqual(2, result[0][1]);
            Assert.AreEqual(4, result[4][0]);
        ...
        {
            double[] data = { 1, 2, 2, 3, 4, 10 };
            var result = data.WindowedEx(2, collected => collected.Sum() >= 4).ToArray();
        ...
        {
            double[] data = { 1, 2, 2, 3, 4, 10 };
            var result = data.MovingAverage(2).ToArray();
            Assert.AreEqual(5, result.Length);
        ...
        public void MovingAverage2()
        {
            ...
            var result = data.MovingAverage(4).ToArray();
        ...
        public void ExponentialMovingAverage()
        {
            ...
            var result = data.ExponentialMovingAverage(10).ToArray();
        ...
        public void WeightedMovingAverage()
        {
            ...
            var result = data.WeightedMovingAverage(4).ToArray();
            Assert.AreEqual(2, result.Length);
        ...
        public void MeanNormalized()
        {
            ...
            var result = data.MeanNormalized().ToArray();
        ...
        {
            double[] data = { 2, 2, 2, 3, 3, 3 };
            var result = data.MeanNormalized(true).ToArray();
            Assert.AreEqual(6, result.Length);
